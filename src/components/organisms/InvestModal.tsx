...
export function InvestModal({ project, isOpen, onClose, walletBalance }: InvestModalProps) {
  ...
  const sharePercent = project.fundingGoal > 0
    ? ((parsedAmount / project.fundingGoal) * project.revenueSharePercent).toFixed(3)
    : '0';
  ...
  const { mutate: invest, isPending } = useMutation({
    mutationFn: () => investmentsApi.invest({ projectId: project.id, amount: parsedAmount }),
    onSuccess: () => {
  ...
  const quickAmounts = [1000, 5000, 10000, 50000].filter((a) => a <= Math.min(walletBalance, remaining));
          ...
          >
            <div className="w-full max-w-md bg-bg-card rounded-3xl border border-border shadow-card overflow-hidden">
              ...
              <div className="relative px-6 pt-6 pb-4 border-b border-border">
                ...
                <h2 className="text-xl font-bold text-text-primary">Инвестировать</h2>
                <p className="text-sm text-text-secondary mt-1 pr-8 truncate">{project.title}</p>
              </div>
              ...
              <div className="p-6 space-y-5">
                      ...
                      <>
                        <div>
                          ...
                          {parsedAmount > 0 && (
                            <div className="mt-1.5 text-xs">
                              {parsedAmount < 100 && (
                                <p className="text-danger">Минимальная сумма — 100 ₸</p>
                              )}
                              {parsedAmount > walletBalance && (
                                <p className="text-danger">Недостаточно средств на кошельке</p>
                              )}
                              {parsedAmount > remaining && parsedAmount <= walletBalance && (
                                <p className="text-danger">Превышает оставшуюся сумму ({formatCurrency(remaining)})</p>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        {isAmountValid && (
                          <div className="p-3 bg-accent-purple/8 border border-accent-purple/20 rounded-2xl">
                            <p className="text-xs text-text-secondary">Ваша доля от выручки составит:</p>
                            <p className="text-lg font-bold text-accent-purple-light mt-0.5">
                              {sharePercent}%
                            </p>
                            <p className="text-xs text-text-muted mt-1">
                              = {parsedAmount.toLocaleString('ru-RU')} ₸ / {formatCurrency(project.fundingGoal)} цель
                            </p>
                          </div>
                        )}
                        ...
                        >
                          {!isAuthenticated ? 'Войдите чтобы инвестировать' : 'Продолжить'}
                        </button>
                      </>
                    )}
                      ...
                      <div className="space-y-4">
                        <div className="p-4 bg-bg-elevated rounded-2xl space-y-2.5">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-text-primary font-bold">{formatCurrency(parsedAmount)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-text-secondary">Доля от выручки</span>
                            <span className="text-accent-cyan font-bold">{sharePercent}%</span>
                          </div>
                          <div className="border-t border-border pt-2.5 flex justify-between text-sm">
                            <span className="text-text-secondary">Остаток баланса</span>
                ...
                {step === 'success' && (
                  <div className="text-center py-4 space-y-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-text-primary">Инвестиция принята!</h3>
                      <p className="text-sm text-text-secondary mt-1">
                        Вы инвестировали {formatCurrency(parsedAmount)} в проект «{project.title}»
                      </p>
                    </div>
                    <button
                      onClick={handleClose}
